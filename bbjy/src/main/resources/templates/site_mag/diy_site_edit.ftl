...
$(function () {
    ...
    $("#cat").change(function(){      
        ...
        $(".cat-title").addClass("hide");
        $(".cat_"+catId).removeClass("hide");
        
        $(".cat-select").removeAttr("name");
        $(".cat-select").removeAttr("datatype");
        
        $("#cat_select_"+catId).attr("name","hostName");
        $("#cat_select_"+catId).attr("datatype","*");
     });  
      ...
      <dd>
           <div class="rule-single-select">
                <select id="roleId" name="roleId" datatype="*" sucmsg=" ">
                     ...
                     <option value="5" <#if diy_site?? && diy_site.roleId?? && diy_site.roleId==5>selected="selected"</#if>>合作机构</option>                   
                </select>
           </div>
       </dd>
  </dl>
...
<#-- 专家归属的投资机构  -->
  <dl  class="invest" <#if !diy_site?? || diy_site?? && diy_site.roleId != 3>style= "display:none;"</#if>>
      <dt>所属投资机构</dt>
      <dd>
           <div class="rule-single-select">
  ...
  <#-- 专家归属的投资机构  end-->
<#-- 合作机构所属类别  -->
  <dl  class="cat" <#if !diy_site?? ||diy_site?? && diy_site.roleId != 5>style= "display:none;"</#if>>
      <dt>所属类别</dt>
      <dd>
           <div class="rule-single-select">
                <select id="cat" name="hostType"  sucmsg=" "  >
                <#if cat_list??>
       				<#list  cat_list as item>   
         				<option value="${item.id?c}" <#if diy_site?? && diy_site.hostType?? && diy_site.hostType==item.id>selected="selected"</#if>>${item.title!''}</option> 
         			</#list>
         		</#if>		
                </select>
           </div>
       <#if cat_list??>
       <#list  cat_list as item>   
       <span class="cat-title cat_${item.id?c} <#if diy_site?? && diy_site.hostType?? && diy_site.hostType==item.id><#else>hide</#if>">机构名称</span>
       <span class="cat-title cat_${item.id?c} <#if diy_site?? && diy_site.hostType?? && diy_site.hostType==item.id><#else>hide</#if> " >
           <div class="rule-single-select">
                <select class="cat-select" id="cat_select_${item.id?c}" <#if diy_site?? && diy_site.hostType?? && diy_site.hostType==item.id>name="hostName"</#if>  sucmsg=" "  >
                     <option value="" <#if !diy_site?? || !diy_site.payType??>selected="selected"</#if>>请选择...</option>
                     <#if ("list_"+item.id)?eval??>
                     <#assign title_list = ("list_"+item.id)?eval>
                     	<#list title_list as article>
                     		<option value="${article.title!''}" <#if diy_site?? && diy_site.hostName?? && diy_site.hostName==article.title>selected="selected"</#if>>${article.title!''}</option> 
          				</#list>
          			</#if>   
                </select>
            </div>    
